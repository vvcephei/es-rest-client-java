...
public class GeoDistanceHelper {
    ...
    private static InternalGeoDistance internalAnonRange(final String name, final List<Object> list, final AggregationsManifest manifest) {
        final ImmutableList.Builder<InternalGeoDistance.Bucket> buckets = ImmutableList.builder();
        for (Object bucketObj : list) {
            final Map<String, Object> bucketMap = nodeMapValue(bucketObj, String.class, Object.class);
            buckets.add(GeoDistanceBucketHelper.fromXContent(null, bucketMap, manifest));
        }
        ...
        return new InternalGeoDistance(name, buckets.build(), ValueFormatter.RAW, true);
    }

    private static InternalGeoDistance internalKeyedRange(final String name, final Map<String, Object> map, final AggregationsManifest manifest) {
        final ImmutableList.Builder<InternalGeoDistance.Bucket> buckets = ImmutableList.builder();
        for (Map.Entry<String, Object> bucketObj : map.entrySet()) {
            ...
            buckets.add(GeoDistanceBucketHelper.fromXContent(bucketName, bucketMap, manifest));
        }
