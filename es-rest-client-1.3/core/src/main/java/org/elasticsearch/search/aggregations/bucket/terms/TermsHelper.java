...
public class TermsHelper {
    ...
    public static InternalTerms fromXContent(final String name, final Map<String, Object> map, final AggregationsManifest subAggregationsManifest) {
        ...
        final InternalOrder internalOrder = InternalOrder.COUNT_DESC; // Note we could try to infer this, supposing the map is an ordered map of some kind.
        ...
        if (isString) {
                return new StringTerms(name, internalOrder, requiredSize, minDocCount, buckets.build());
        } else if (isLong) {
            return new LongTerms(name, internalOrder, valueFormatter, requiredSize, minDocCount, buckets.build());
        } else if (isDouble){
            return new DoubleTerms(name, internalOrder, valueFormatter, requiredSize, minDocCount, buckets.build());
        } else {
